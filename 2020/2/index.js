...
const data = inputs.map(e =>{ const parts = e.split(':'); return [...parts[0].split('-').map(e => parseInt(e)), ...parts[0][parts[0].length-1], parts[1].trim()] });

const result1 = data.filter(([min, max, char, str]) => {
...
const result2 = data.filter(([i1, i2, char, str]) => {
    const isFirst = str[i1-1] === char;
    const isSecond = str[i2-1] === char;
    return ((isFirst ? 1 : 0) + (isSecond ? 1 : 0)) === 1
})
